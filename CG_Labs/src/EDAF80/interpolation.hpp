...
namespace interpolation
{
	...
	//! \brief interpolates a position between two or more points.
	//!
	//! @param [in] ps points for the interpolation
	//! @param [in] n of the points array
	//! @param [in] x distance ratio between p0 and pn-1 at which the
	//!               interpolated point should be:
	//!               * x == 0.0: result will be pi;
	//!               * x == 1.0: result will be pi+1;
	//!               * x == 0.5: result will be the midpoint of [pi,pi+1]
	//!               * x ∈ ]0,1[: result will be somewhere on ]pi,pi+1[
	//! @return interpolated position
	glm::vec3 evalLERP(const glm::vec3* ps, unsigned int n, float const x);
