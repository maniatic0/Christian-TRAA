...
Node::get_transform() const
{
	...
	auto const ans = translating * rotating * scaling;
...
void Node::update(InputHandler* inputHandler, const float &dt) {
	return;
}


void Node::UpdateNodes(InputHandler* inputHandler, const float& dt) {
	auto current_node = Node::nodeList.begin();

	while (current_node != Node::nodeList.end())
	{
		if ((*current_node)->active)
		{
			(*current_node)->update(inputHandler, dt);
		}
		current_node++;
	}
