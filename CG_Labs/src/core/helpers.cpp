...
GLuint
bonobo::createProgram(std::string const& vert_shader_source_path, std::string const& frag_shader_source_path, std::string const& macros)
{
	auto const vertex_shader_source = macros + utils::slurp_file(config::shaders_path("EDAF80/" + vert_shader_source_path));
	GLuint vertex_shader = utils::opengl::shader::generate_shader(GL_VERTEX_SHADER, vertex_shader_source);
	if (vertex_shader == 0u)
		return 0u;

	auto const fragment_shader_source = macros + utils::slurp_file(config::shaders_path("EDAF80/" + frag_shader_source_path));
...
void bonobo::saveConfig(std::string file_name, const bool &using_other_aa, const int &aa_num,
	...
	const bool &ghosting_test, const int &ghosting_test_amount)
{
	...
	log_file << "Using Other AA: " << using_other_aa << std::endl;
	log_file << "AA Number: " << aa_num << std::endl;
